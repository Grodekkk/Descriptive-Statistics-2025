...
```python
import pandas as pd
url = "https://raw.githubusercontent.com/Grodekkk/pandas_exercises/refs/heads/main/data/pokemon.csv"
...
```
    ...
    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\io\parsers\readers.py:620, in _read(filepath_or_buffer, kwds)
    ...
    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\io\parsers\readers.py:1620, in TextFileReader.__init__(self, f, engine, **kwds)
       1617     self.options["has_index_names"] = kwds["has_index_names"]
       1619 self.handles: IOHandles | None = None
    -> 1620 self._engine = self._make_engine(f, self.engine)
    

    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\io\parsers\readers.py:1880, in TextFileReader._make_engine(self, f, engine)
    ...
    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\io\common.py:873, in get_handle(path_or_buf, mode, encoding, compression, memory_map, is_text, errors, storage_options)
...
```python
#Calculate STD for the SPEED of legendary and non legendary pokemon
df_pokemon
legendary_pokemon = df_pokemon[df_pokemon['Legendary'] == True]
non_legendary_pokemon = df_pokemon[df_pokemon['Legendary'] == False]

mean = np.mean(legendary_pokemon['Speed'])

var_sample = stats.tvar(legendary_pokemon['Speed'])      # sample variance
std_sample = stats.tstd(legendary_pokemon['Speed'])      # sample sd
...
print(f"Sample variance (scipy): {var_sample}")
print(f"Sample sd (scipy): {std_sample}")
print(f"legendary (scipy): {cv_sample:.2f}%")


mean = np.mean(non_legendary_pokemon['Speed'])

var_sample = stats.tvar(non_legendary_pokemon['Speed'])      # sample variance
std_sample = stats.tstd(non_legendary_pokemon['Speed'])      # sample sd
...
print(f"Sample variance (scipy): {var_sample}")
print(f"Sample sd (scipy): {std_sample}")
print(f"non-legendary (scipy): {cv_sample:.2f}%")


```

    Mean: 100.18461538461538
    Sample variance (scipy): 526.8091346153847
    Sample sd (scipy): 22.952323076660118
    legendary (scipy): 22.91%
    Mean: 65.45578231292517
    Sample variance (scipy): 775.2347587536378
    Sample sd (scipy): 27.84303788658195
    non-legendary (scipy): 42.54%
...
skewness = skew(non_legendary_pokemon['Speed'])
print(f"Skewness of non legendary: {skewness:.4f}")

skewness = skew(legendary_pokemon['Speed'])
print(f"Skewness of legendary: {skewness:.4f}")
```

    Skewness of non legendary: 0.3893
    Skewness of legendary: 0.4321
...
```python
# your plots and comments here by generation <?>
```
...
import pandas as pd
from scipy.stats import skew
url = "https://github.com/kflisikowski/ds/blob/master/danex.csv?raw=true"
mydata1 = pd.read_csv(url, sep=';', decimal=',')
mydata1.head()

deviation = mydata1.std()
print(deviation, "<---")
mean1 = mydata1.mean()
print(mean1, "<---")

skewness1 = skew(mydata1)
print("skewness: ", skewness1)

min1 = min(mydata1)
print("min: ", min1)

median1 = np.median(mydata1)
print("miedian: ", median1)
...
# Quantiles
q1 = s.quantile(0.26) # lower quartile Q1
median = s.quantile(0.5) # median or middle quartile Q2 = Me
q3 = s.quantile(0.74) # upper quartile Q3
...
```
    ...
    Cell In[26], line 26
         22 print("miedian: ", median1)
         25 # Conversion to Pandas Series
    ---> 26 s = pd.Series(mydata1)
         28 # Quantiles
         29 q1 = s.quantile(0.26) # lower quartile Q1
    

    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\core\series.py:584, in Series.__init__(self, data, index, dtype, name, copy, fastpath)
        582         data = data.copy()
        583 else:
    ...
    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\core\construction.py:633, in sanitize_array(data, index, dtype, copy, allow_2d)
        631     else:
    ...
    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\core\construction.py:659, in sanitize_array(data, index, dtype, copy, allow_2d)
    ...
    File c:\Users\grode\AppData\Local\Programs\Python\Python313\Lib\site-packages\pandas\core\construction.py:718, in _sanitize_ndim(result, data, dtype, index, allow_2d)
        716     if allow_2d:
        717         return result
    ...
    ValueError: Data must be 1-dimensional, got ndarray of shape (31, 1) instead
